...
public class Aerolinea {
    ...
    public void programarVuelo(String fecha, String codigoRuta, String nombreAvion) throws Exception {
        ...
        if (avionAsignado == null) {
            throw new Exception("El avión con nombre " + nombreAvion + " no existe en la aerolínea.");
        }

        for (Vuelo v : vuelos) {
            if (v.getAvion().getNombre().equals(nombreAvion) && v.getFecha().equals(fecha)) {
                throw new Exception(
                        "El avión " + nombreAvion + " ya tiene un vuelo programado para la fecha " + fecha + ".");
            }
        }

        Vuelo nuevoVuelo = new Vuelo(rutaVuelo, fecha, avionAsignado);
    ...
    public int venderTiquetes(String identificadorCliente, String fecha, String codigoRuta, int cantidad)
        ...
        if (cliente == null) {
            throw new Exception("El cliente " + identificadorCliente + " no está registrado.");
        }

        Vuelo vuelo = getVuelo(codigoRuta, fecha);
        if (vuelo == null) {
            throw new Exception(
                    "No hay ningún vuelo programado para la ruta " + codigoRuta + " en la fecha " + fecha + ".");
        }
    ...
    public void registrarVueloRealizado(String fecha, String codigoRuta) {
        Vuelo vueloPasado = getVuelo(codigoRuta, fecha);
        if (vueloPasado != null) {
            for (Tiquete t : vueloPasado.getTiquetes()) {
                t.getCliente().usarTiquetes(vueloPasado);
            }
        }
    }
